...
class Camera: NSObject {
    ...
    private func configureCaptureSession(completionHandler : (_ success : Bool)->Void){
        ...
        else{logger.error("failed to obtain video input")
            return
        }
...
extension Camera : AVCapturePhotoCaptureDelegate{
    func photoOutput(_ output : AVCapturePhotoOutput,didFinidhProcessingPhoto photo : AVCapturePhoto,error : Error?){
        if let error = error{
            logger.error("Error capture photo: \(error.localizedDescription)")
            return
        }
...
fileprivate let logger = Logger(subsystem: "co.innerVibe", category: "Camera")
